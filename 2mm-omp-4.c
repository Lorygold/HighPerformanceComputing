                       ...
                       DATA_TYPE POLYBENCH_2D(D, NI, NL, ni, nl))
{
  ...
  /* D := alpha*A*B*C + beta*D */
  {
    ...
    #pragma omp target parallel for map(to:A[0:_PB_NI][0:_PB_NK], B[0:_PB_NK][0:_PB_NJ]) map(tofrom:tmp[0:_PB_NI][0:_PB_NJ]) collapse(2)
    for (i = 0; i < _PB_NI; i++)
      for (j = 0; j < _PB_NJ; j++)
      {
        tmp[i][j] = 0;
    ...
    #pragma omp target parallel for map(to:tmp[0:_PB_NI][0:_PB_NJ], C[0:_PB_NJ][0:_PB_NL]) map(tofrom:D[0:_PB_NI][0:_PB_NL]) collapse(2)
    for (i = 0; i < _PB_NI; i++)
      for (j = 0; j < _PB_NL; j++)
      {
        D[i][j] *= beta;
