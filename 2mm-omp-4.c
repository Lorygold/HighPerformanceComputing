                       ...
                       DATA_TYPE POLYBENCH_2D(D, NI, NL, ni, nl))
{
  ...
  {
    #pragma omp target teams map(to:A[0:_PB_NI][0:_PB_NK], B[0:_PB_NK][0:_PB_NJ]) map(tofrom:tmp[0:_PB_NI][0:_PB_NJ])
    ...
    #pragma omp target teams map(to:tmp[0:_PB_NI][0:_PB_NJ], C[0:_PB_NJ][0:_PB_NL]) map(tofrom:D[0:_PB_NI][0:_PB_NL])
    #pragma omp distribute parallel for collapse(2)
    for (i = 0; i < _PB_NI; i++)
      for (j = 0; j < _PB_NL; j++)
      {
